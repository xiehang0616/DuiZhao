...
export function createAnswerStream({modelIds,onUpdate,onSettled,schedule=requestAnimationFrame,cancel=cancelAnimationFrame,random=Math.random,reduced=()=>false}){
 const slots=modelIds.map(id=>({id,chars:[],shown:0,text:'',status:'waiting',terminal:null,next:0,error:null,source:null,media:null,usage:null,totalMs:null,firstTokenMs:null}));
 let frame=null,closed=false;
 const snapshots=()=>slots.map(s=>({text:s.text,status:s.status,error:s.error,source:s.source,media:s.media,usage:s.usage,totalMs:s.totalMs,firstTokenMs:s.firstTokenMs}));
 ...
 return {
  ...
  end(id,{error=null,fullText,source=null,media=null,usage=null,totalMs=null,firstTokenMs=null}={}){forModel(id,s=>{if(typeof fullText==='string'&&fullText.startsWith(s.chars.join('')))s.chars=Array.from(fullText);s.terminal=error?'error':'done';s.error=error;s.source=source;s.media=media;s.usage=usage;s.totalMs=totalMs;s.firstTokenMs=firstTokenMs})},
