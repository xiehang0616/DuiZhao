...
export function ComposerSurface({attachments,disabled,children}){
 ...
 {attachments.items.length>0&&<><AttachmentList items={attachments.items} onRemove={attachments.remove} disabled={disabled}/>{attachments.items.some(a=>a.type!=='text')&&<p className="attachment-support-note" role="status">图片附件可在文本模式发送用于图片理解；视频、文档等其他文件暂不支持发送。</p>}</>}{children}</div>;
}
