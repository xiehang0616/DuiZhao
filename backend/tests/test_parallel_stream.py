...
def test_models_start_in_parallel_and_finish_independently(monkeypatch):
    ...
    async def fake(model, question, prompt, stats, images=None):
        ...
        yield '## ' + model['id']
        await asyncio.sleep(.03 if model['id'] == 'qwen3.8-flash' else .001)
        yield '完成'
    monkeypatch.setattr(main, 'stream_completion', fake)
    tid = client.post('/api/v1/compare', json={'question':'并行', 'modelIds':['qwen3.8-flash','deepseek-chat']}).json()['taskId']
    events = parse(client.get(f'/api/v1/compare/{tid}/events').text)
    first_done = next(i for i,e in enumerate(events) if e[0] == 'done')
    assert {e[1]['modelId'] for e in events[:first_done] if e[0] == 'chunk'} == {'qwen3.8-flash','deepseek-chat'}
    assert events[first_done][1]['modelId'] == 'deepseek-chat'
...
def test_failure_does_not_discard_peer_or_partial_text(monkeypatch):
    async def fake(model, question, prompt, stats, images=None):
        ...
        yield '成功'
    monkeypatch.setattr(main, 'stream_completion', fake)
    tid = client.post('/api/v1/compare', json={'question':'错误隔离', 'modelIds':['qwen3.8-flash','deepseek-chat']}).json()['taskId']
    events = parse(client.get(f'/api/v1/compare/{tid}/events').text)
    assert any(e == 'done' and d['modelId'] == 'deepseek-chat' for e,d in events)
    results = {r['modelId']:r for r in client.get(f'/api/v1/compare/{tid}').json()['results']}
    assert results['qwen3.8-flash']['fullText'] == '已收到'
...
def test_cancel_interrupts_waiting_providers_and_saves_partial_text(monkeypatch):
    ...
    monkeypatch.setattr(main, 'stream_completion', fake)
    async def run():
        req = main.CompareRequest(question='取消', modelIds=['qwen3.8-flash','deepseek-chat'])
